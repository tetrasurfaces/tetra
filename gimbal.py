...
# Button for hex_mode
ax_hex = plt.axes([0.8, 0.025, 0.1, 0.075])
...
def update(val):
    ...
    morph = s_morph.val
    hex_mode = init_hex_mode  # Will be toggled separately
    ...
    if hex_mode and X_cap is not None:
        ax.plot_surface(X_cap, Y_cap, Z_cap, cmap='plasma', linewidth=0, antialiased=False)
    ax.set_title(f'NURKS Surface (ID: {surface_id})')
    ax.set_xlim(np.min(X), np.max(X))
    ax.set_ylim(np.min(Y), np.max(Y))
    ax.set_zlim(np.min(Z), np.max(Z))
